(function ($) {
  var addCloseCartBind = function ($cart, $cart_button, $menu) {
    $("body").on("mouseover.ope-woo", function (event) {
      ...
      var related =
        isHeaderCartRelated($target, $cart, $cart_button) || $target.is($menu);
      if (!related) {
  ...
  function addCloseCartButton($cart) {
    $cart.prepend(
      '<a href="#" class="close-mini-cart small"><i class="fa fa-close"></i></a>'
    );
  ...
  function showCart($cart, $cart_button) {
    ...
    var top =
      $cart_button.offset().top +
      $cart_button.outerHeight() -
      $cart_button.closest("div").offset().top;
  ...
  function isHeaderCartRelated($target, $cart, $cart_button) {
    var isMenuButtoRelated =
      $.contains($cart_button[0], $target[0]) || $target.is($cart_button);
    var isCartContentRelated =
      $.contains($cart[0], $target[0]) || $target.is($cart);
